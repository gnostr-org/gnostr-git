...
impl AsyncDiff {
    ...
    fn get_diff_helper(
        ...
        let res = match params.diff_type {
            DiffType::Stage => sync::diff::get_diff(repo_path, &params.path, true, Some(params.options))?,
            DiffType::WorkDir => sync::diff::get_diff(repo_path, &params.path, false, Some(params.options))?,
            DiffType::Commit(id) => {
                sync::diff::get_diff_commit(repo_path, id, params.path.clone(), Some(params.options))?
            }
            DiffType::Commits(ids) => {
                sync::diff::get_diff_commits(repo_path, ids, params.path.clone(), Some(params.options))?
            }
        };
...

        {
            ...
            *last = Some(LastResult { result: res, params });
        }
