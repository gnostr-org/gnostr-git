...
enum JobState {
    Request { local_branches: bool, repo: RepoPath },
...
impl AsyncBranchesJob {
    ...
    pub fn new(repo: RepoPath, local_branches: bool) -> Self {
        Self {
            state: Arc::new(Mutex::new(Some(JobState::Request { repo, local_branches }))),
        }
    }
...
impl AsyncJob for AsyncBranchesJob {
    ...
    fn run(&mut self, _params: RunParams<Self::Notification, Self::Progress>) -> Result<Self::Notification> {
        if let Ok(mut state) = self.state.lock() {
            *state = state.take().map(|state| match state {
                JobState::Request { local_branches, repo } => {
