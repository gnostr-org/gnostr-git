...
impl RemoteProgress {
    ...
    pub(crate) fn set_progress<T>(progress: &Arc<Mutex<Option<T>>>, state: Option<T>) -> Result<()> {
    ...
    pub(crate) fn spawn_receiver_thread<T: 'static + AsyncProgress>(
        ...
        thread::spawn(move || loop {
            ...
            match incoming {
                Ok(update) => {
                    Self::set_progress(&progress, Some(update.clone())).expect("set progress failed");
...
impl From<ProgressNotification> for RemoteProgress {
    fn from(progress: ProgressNotification) -> Self {
        match progress {
            ProgressNotification::Packing { stage, current, total } => match stage {
                PackBuilderStage::AddingObjects => Self::new(RemoteProgressState::PackingAddingObject, current, total),
                PackBuilderStage::Deltafication => Self::new(RemoteProgressState::PackingDeltafiction, current, total),
            },
