...
pub fn get_diff(repo_path: &RepoPath, p: &str, stage: bool, options: Option<DiffOptions>) -> Result<FileDiff> {
    scope_time!("get_diff");
...
fn raw_diff_to_file_diff(diff: &Diff, work_dir: &Path) -> Result<FileDiff> {
    let res = Rc::new(RefCell::new(FileDiff::default()));
    {
            ...
            if let Some(delta) = diff.deltas().next() {
                if delta.status() == Delta::Untracked {
                    let relative_path = delta
                        .new_file()
                        .path()
                        .ok_or_else(|| Error::Generic("new file path is unspecified.".to_string()))?;

                    let newfile_path = work_dir.join(relative_path);

                    if let Some(newfile_content) = new_file_content(&newfile_path) {
                        let mut patch =
                            Patch::from_buffers(&[], None, newfile_content.as_slice(), Some(&newfile_path), None)?;

                        patch.print(&mut |delta, hunk: Option<DiffHunk>, line: git2::DiffLine| {
                            put(delta, hunk, line);
                            true
                        })?;
        ...
        if !new_file_diff {
            diff.print(DiffFormat::Patch, move |delta, hunk, line: git2::DiffLine| {
                put(delta, hunk, line);
                true
            })?;
        }
...
1   end";
    ...
    fn test_hunks() {
        ...
        {
            File::create(&file_path).unwrap().write_all(HUNK_A.as_bytes()).unwrap();
        }
        ...
        {
            File::create(&file_path).unwrap().write_all(HUNK_B.as_bytes()).unwrap();
        }
    ...
    fn test_diff_newfile_in_sub_dir_current_dir() {
        ...
        File::create(root.join(file_path)).unwrap().write_all(b"test").unwrap();
