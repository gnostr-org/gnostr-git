...
mod tests {
    ...
    use crate::{
        error::Result,
        sync::{commit, stage_add_file, tests::repo_init_empty, utils::get_head_repo, CommitId, RepoPath},
    };
    ...
    fn test_get_commit_from_revision() -> Result<()> {
        ...
        assert_eq!(CommitId::from_revision(repo_path, c1_rev.as_str()).unwrap(), c1);
