...
///
pub fn blame_file(repo_path: &RepoPath, file_path: &str, commit_id: Option<CommitId>) -> Result<FileBlame> {
    ...
    let spec = format!("{}:{}", commit_id.to_string(), fixup_windows_path(file_path));
    ...
    let unique_commit_ids: HashSet<_> = blame.iter().map(|hunk| CommitId::new(hunk.final_commit_id())).collect();
