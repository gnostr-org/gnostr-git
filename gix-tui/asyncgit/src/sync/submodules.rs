...
pub fn get_submodules(repo_path: &RepoPath) -> Result<Vec<SubmoduleInfo>> {
    ...
    let res = r.submodules()?.iter().map(|s| submodule_to_info(s, &repo2)).collect();
...
pub fn submodule_parent_info(repo_path: &RepoPath) -> Result<Option<SubmoduleParentInfo>> {
    ...
    if let Some(parent_path) = repo_wd.parent() {
        ...
        if let Ok(parent) = Repository::open_ext(parent_path, RepositoryOpenFlags::empty(), Vec::<&Path>::new()) {
            let parent_wd = work_dir(&parent)?.to_path_buf();
            log::trace!("[sub] parent_wd: {:?}", parent_wd);

            let submodule_name = repo_wd.strip_prefix(parent_wd)?.to_string_lossy().to_string();
