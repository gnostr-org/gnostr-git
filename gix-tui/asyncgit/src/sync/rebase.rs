...
pub fn rebase_branch(repo_path: &RepoPath, branch: &str, branch_type: BranchType) -> Result<RebaseState> {
...
fn rebase_branch_repo(repo: &Repository, branch_name: &str, branch_type: BranchType) -> Result<RebaseState> {
...
pub fn conflict_free_rebase(repo: &git2::Repository, commit: &git2::AnnotatedCommit) -> Result<CommitId> {
...
mod test_conflict_free_rebase {
    ...
    fn test_rebase_branch_repo(repo_path: &RepoPath, branch_name: &str) -> CommitId {
        ...
        let annotated = repo.reference_to_annotated_commit(&branch.into_reference()).unwrap();
