...
use crate::{
    error::Result,
    sync::{
        diff::DiffLinePosition, patches::get_file_diff_patch_and_hunklines, repository::repo, utils::repo_write_file,
        RepoPath,
    },
};

/// discards specific lines in an unstaged hunk of a diff
pub fn discard_lines(repo_path: &RepoPath, file_path: &str, lines: &[DiffLinePosition]) -> Result<()> {
