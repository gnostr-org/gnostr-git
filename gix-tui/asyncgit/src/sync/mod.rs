...
pub use branch::{
    branch_compare_upstream, checkout_branch, checkout_commit, config_is_pull_rebase, create_branch, delete_branch,
    get_branch_remote, get_branches_info, merge_commit::merge_upstream_commit,
    merge_ff::branch_merge_upstream_fastforward, merge_rebase::merge_upstream_rebase, rename::rename_branch,
...
pub use commit_filter::{
    diff_contains_file, filter_commit_by_search, LogFilterSearch, LogFilterSearchOptions, SearchFields, SearchOptions,
...
pub use hooks::{
    hooks_commit_msg, hooks_post_commit, hooks_pre_commit, hooks_prepare_commit_msg, HookResult, PrepareCommitMsgSource,
};
pub use hunks::{reset_hunk, stage_hunk, unstage_hunk};
pub use ignore::add_to_ignore;
pub use logwalker::LogWalker;
pub use merge::{
    abort_pending_rebase, abort_pending_state, continue_pending_rebase, merge_branch, merge_commit, merge_msg,
    mergehead_ids, rebase_progress,
};
pub use rebase::rebase_branch;
pub use remotes::{get_default_remote, get_remotes, push::AsyncProgress, tags::PushTagsProgress};
...
pub use submodules::{
    get_submodules, submodule_parent_info, update_submodule, SubmoduleInfo, SubmoduleParentInfo, SubmoduleStatus,
};
pub use tags::{delete_tag, get_tags, get_tags_with_metadata, CommitTags, Tag, TagWithMetadata, Tags};
pub use tree::{tree_file_content, tree_files, TreeFile};
pub use utils::{
    get_head, get_head_tuple, repo_dir, repo_open_error, stage_add_all, stage_add_file, stage_addremoved, Head,
};
...
mod tests {
    ...
    pub fn write_commit_file(repo: &Repository, file: &str, content: &str, commit_name: &str) -> CommitId {
        repo_write_file(repo, file, content).unwrap();

        stage_add_file(&repo.workdir().unwrap().to_str().unwrap().into(), Path::new(file)).unwrap();

        commit(&repo.workdir().unwrap().to_str().unwrap().into(), commit_name).unwrap()
    }
    ...
    pub fn get_statuses(repo_path: &RepoPath) -> (usize, usize) {
        (
            get_status(repo_path, StatusType::WorkingDir, None).unwrap().len(),
            get_status(repo_path, StatusType::Stage, None).unwrap().len(),
        )
    }
    ...
    pub fn get_commit_ids(r: &Repository, max_count: usize) -> Vec<CommitId> {
        ...
        LogWalker::new(r, max_count).unwrap().read(&mut commit_ids).unwrap();
