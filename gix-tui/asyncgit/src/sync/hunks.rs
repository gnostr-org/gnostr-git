...
///
pub fn stage_hunk(repo_path: &RepoPath, file_path: &str, hunk_hash: u64, options: Option<DiffOptions>) -> Result<()> {
...
pub fn reset_hunk(repo_path: &RepoPath, file_path: &str, hunk_hash: u64, options: Option<DiffOptions>) -> Result<()> {
...
pub fn unstage_hunk(
    ...
    let hunk_index = find_hunk_index(&diff, hunk_hash);
    let hunk_index = hunk_index.map_or_else(|| Err(Error::Generic("hunk not found".to_string())), Ok)?;
...
mod tests {
    ...
    fn reset_untracked_file_which_will_not_find_hunk() -> Result<()> {
        ...
        assert!(reset_hunk(repo_path, file_path.to_str().unwrap(), diff.hunks[0].header_hash, None,).is_err());
