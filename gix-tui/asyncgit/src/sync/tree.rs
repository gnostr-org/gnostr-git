...
///
fn tree_recurse(repo: &Repository, path: &Path, tree: &Tree, out: &mut Vec<TreeFile>) -> Result<()> {
...
mod tests {
    ...
    fn test_sorting() {
        ...
        assert_eq!(
            list.iter().map(|f| f.path.to_string_lossy()).collect::<Vec<_>>(),
            vec![
    ...
    fn test_sorting_folders2() {
        ...
        assert_eq!(
            list.iter().map(|f| f.path.to_string_lossy()).collect::<Vec<_>>(),
            vec![String::from("afolder/file"), String::from("bfolder/sub/file"),]
        );
    }
    ...
    fn test_path_cmp() {
        assert_eq!(
            path_cmp(&PathBuf::from("bfolder/sub/file"), &PathBuf::from("afolder/file")),
            Ordering::Greater
        );
    }
