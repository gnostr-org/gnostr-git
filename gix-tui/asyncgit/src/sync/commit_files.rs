...
pub fn get_compare_commits_diff(
    ...
    let commits = (repo.find_commit(ids.0.into())?, repo.find_commit(ids.1.into())?);
...
pub(crate) fn get_commit_diff<'a>(
    ...
    let parent = if commit.parent_count() > 0 {
        repo.find_commit(commit.parent_id(0)?).ok().and_then(|c| c.tree().ok())
    } else {
        None
    };
    ...
    if stashes.map(|stashes| stashes.contains(&id)).unwrap_or_default() {
        if let Ok(untracked_commit) = commit.parent_id(2) {
            let untracked_diff = get_commit_diff(repo, CommitId::new(untracked_commit), pathspec, options, stashes)?;
