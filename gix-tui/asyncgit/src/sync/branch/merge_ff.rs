...
///
pub fn branch_merge_upstream_fastforward(repo_path: &RepoPath, branch: &str) -> Result<()> {
    ...
    if !analysis.is_fast_forward() {
        return Err(Error::Generic("fast forward merge not possible".into()));
    }
...
pub mod test {
    ...
    use crate::sync::{
        ...
        tests::{debug_cmd_print, get_commit_ids, repo_clone, repo_init_bare, write_commit_file},
    };
    ...
    fn test_merge_fastforward() {
        ...
        // clone2
        debug_cmd_print(&clone2_dir.path().to_str().unwrap().into(), "git pull --ff");

        let commit2 = write_commit_file(&clone2, "test2.txt", "test", "commit2");
        ...
        let bytes = fetch(&clone1_dir.path().to_str().unwrap().into(), "master", None, None).unwrap();
        assert!(bytes > 0);

        let bytes = fetch(&clone1_dir.path().to_str().unwrap().into(), "master", None, None).unwrap();
        assert_eq!(bytes, 0);

        branch_merge_upstream_fastforward(&clone1_dir.path().to_str().unwrap().into(), "master").unwrap();
