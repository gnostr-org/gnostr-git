...
use super::{remotes::get_default_remote_in_repo, utils::bytes2string, RepoPath};
use crate::{
    error::{Error, Result},
...
///
pub(crate) fn branch_set_upstream(repo: &Repository, branch_name: &str) -> Result<()> {
...
pub fn checkout_branch(repo_path: &RepoPath, branch_name: &str) -> Result<()> {
    ...
    repo.checkout_tree(target_treeish_object, Some(&mut git2::build::CheckoutBuilder::new()))?;
...
pub fn delete_branch(repo_path: &RepoPath, branch_ref: &str) -> Result<()> {
    ...
    if branch.is_head() {
        return Err(Error::Generic(
            "You cannot be on the branch you want to delete, switch branch, then delete this branch".to_string(),
        ));
    }
    branch.delete()?;
    Ok(())
...
mod tests_branches {
    ...
    fn test_remotes_of_branches() {
        ...
        assert_eq!(branches[1].local_details().unwrap().remote.as_ref().unwrap(), "r1");
        assert_eq!(branches[2].local_details().unwrap().remote.as_ref().unwrap(), "r2");
...
mod test_remote_branches {
    ...
    fn test_remote_branches() {
        ...
        push_branch(&clone1_dir.into(), "origin", "master", false, false, None, None).unwrap();

        create_branch(&clone1_dir.into(), "foo").unwrap();

        write_commit_file(&clone1, "test.txt", "test2", "commit2");

        push_branch(&clone1_dir.into(), "origin", "foo", false, false, None, None).unwrap();
    ...
    fn test_checkout_remote_branch() {
        ...
        assert_eq!(get_branches_info(&clone2_dir.into(), true).unwrap().len(), 2);
    ...
    fn test_checkout_remote_branch_hirachical() {
        ...
        write_commit_file(&clone1, "test.txt", "test2", "commit2");
        push_branch(&clone1_dir.into(), "origin", branch_name, false, false, None, None).unwrap();
