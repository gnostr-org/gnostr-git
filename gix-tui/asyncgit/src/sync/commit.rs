...
pub(crate) fn signature_allow_undefined_name(repo: &Repository) -> std::result::Result<Signature<'_>, git2::Error> {
    ...
    if let Err(ref e) = signature {
        if e.code() == ErrorCode::NotFound {
            ...
            if let (Err(_), Ok(email_entry)) = (config.get_entry("user.name"), config.get_entry("user.email")) {
...
pub fn tag_commit(repo_path: &RepoPath, commit_id: &CommitId, tag: &str, message: Option<&str>) -> Result<CommitId> {
...
mod tests {
    ...
    fn test_tag_with_message() -> Result<()> {
        ...
        assert_eq!(
            get_tags(repo_path).unwrap()[&new_id][0].annotation.as_ref().unwrap(),
            "tag-message"
        );
