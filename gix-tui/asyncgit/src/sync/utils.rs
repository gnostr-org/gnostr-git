...
pub fn repo_open_error(repo_path: &RepoPath) -> Option<String> {
    Repository::open_ext(repo_path.gitpath(), RepositoryOpenFlags::empty(), Vec::<&Path>::new())
...
mod tests {
    ...
    fn test_staging_folder() -> Result<()> {
        ...
        let status_count = |s: StatusType| -> usize { get_status(repo_path, s, None).unwrap().len() };
