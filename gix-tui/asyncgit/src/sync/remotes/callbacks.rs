...
impl Callbacks {
    ///
    pub fn new(sender: Option<Sender<ProgressNotification>>, basic_credential: Option<BasicAuthCredential>) -> Self {
    ...
    pub fn callbacks<'a>(&self) -> RemoteCallbacks<'a> {
        ...
        callbacks.sideband_progress(move |data| {
            log::debug!("sideband transfer: '{}'", String::from_utf8_lossy(data).trim());
            true
        });
    ...
    fn pack_progress(&self, stage: git2::PackBuilderStage, total: usize, current: usize) {
        ...
        self.sender
            .clone()
            .map(|sender| sender.send(ProgressNotification::Packing { stage, total, current }));
    }
    ...
    fn push_transfer_progress(&self, current: usize, total: usize, bytes: usize) {
        ...
        self.sender
            .clone()
            .map(|sender| sender.send(ProgressNotification::PushTransfer { current, total, bytes }));
    }
    ...
    fn credentials(
        ...
        log::debug!("creds: '{}' {:?} ({:?})", url, username_from_url, allowed_types);
