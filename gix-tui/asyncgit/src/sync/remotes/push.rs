...
impl AsyncProgress for ProgressNotification {
    ...
    fn progress(&self) -> ProgressPercent {
        match *self {
            Self::Packing { stage, current, total } => match stage {
...
pub fn push_raw(
    ...
    if let Some((reference, msg)) = callbacks.get_stats()?.push_rejected_msg {
        return Err(Error::Generic(format!("push to '{reference}' rejected: {msg}")));
    }
...
mod tests {
    ...
    fn test_force_push() {
        ...
        sync::commit(&tmp_repo_dir.path().to_str().unwrap().into(), "repo_1_commit").unwrap();
        ...
        sync::commit(&tmp_other_repo_dir.path().to_str().unwrap().into(), "repo_2_commit").unwrap();
    ...
    fn test_force_push_rewrites_history() {
        ...
        let repo_1_commit = sync::commit(&tmp_repo_dir.path().to_str().unwrap().into(), "repo_1_commit").unwrap();

        //NOTE: make sure the commit actually contains that file
        assert_eq!(
            sync::get_commit_files(&tmp_repo_dir.path().to_str().unwrap().into(), repo_1_commit, None).unwrap()[0].path,
        ...
        let repo_2_commit = sync::commit(&tmp_other_repo_dir.path().to_str().unwrap().into(), "repo_2_commit").unwrap();
