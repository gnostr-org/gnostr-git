...
use crate::{
    error::{Error, Result},
    sync::{cred::BasicAuthCredential, remotes::push::ProgressNotification, repository::repo, utils},
    ProgressPercent,
};
...
mod tests {
    ...
    fn test_default_remote() {
        ...
        debug_cmd_print(repo_path, &format!("git remote add second {remote_path}")[..]);

        let remotes = get_remotes(repo_path).unwrap();

        assert_eq!(remotes, vec![String::from("origin"), String::from("second")]);
    ...
    fn test_default_remote_out_of_order() {
        ...
        debug_cmd_print(repo_path, &format!("git remote add origin {remote_path}")[..]);

        //NOTE: aparently remotes are not chronolically sorted but alphabetically
        let remotes = get_remotes(repo_path).unwrap();

        assert_eq!(remotes, vec![String::from("alternate"), String::from("origin")]);
    ...
    fn test_default_remote_inconclusive() {
        ...
        debug_cmd_print(repo_path, &format!("git remote add someremote {remote_path}")[..]);
