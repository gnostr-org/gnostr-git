...
fn remote_tag_refs(
    ...
    let conn = remote.connect_auth(Direction::Fetch, Some(callbacks.callbacks()), Some(proxy_auto()))?;
...
pub fn push_tags(
    ...
    for (idx, tag) in tags_missing.into_iter().enumerate() {
        ...
        progress_sender
            .as_ref()
            .map(|sender| sender.send(PushTagsProgress::Push { pushed: idx + 1, total }));
    }
...
mod tests {
    ...
    fn test_tags_delete_remote() {
        ...
        push_raw(clone1_dir, "origin", "tag1", PushType::Tag, false, true, None, None).unwrap();
