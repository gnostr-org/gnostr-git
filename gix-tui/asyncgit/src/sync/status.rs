...
///
pub fn is_workdir_clean(repo_path: &RepoPath, show_untracked: Option<ShowUntrackedFilesConfig>) -> Result<bool> {
    ...
    for e in statuses.iter() {
        ...
        let path = match e.head_to_index() {
            Some(diff) => diff
                ...
                .map(String::from)
                .ok_or_else(|| Error::Generic("failed to get path to diff's new file.".to_string()))?,
            None => e
                .path()
                .map(String::from)
                .ok_or_else(|| Error::Generic("failed to get the path to indexed file.".to_string()))?,
        };
