...
impl CommitMessage {
    ...
    pub fn from(s: &str) -> Self {
        ...
        let subject = lines.next().map_or_else(String::new, std::string::ToString::to_string);
...
pub fn get_commit_details(repo_path: &RepoPath, id: CommitId) -> Result<CommitDetails> {
    ...
    let committer = if author == committer { None } else { Some(committer) };
...
mod tests {
    ...
    fn test_msg_invalid_utf8() -> Result<()> {
        ...
        assert_eq!(res.message.as_ref().unwrap().subject.starts_with("test msg"), true);
