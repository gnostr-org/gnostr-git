    ...
    repo.tag_foreach(|id, name| {
            ...
            String::from_utf8(name[10..name.len()].into())
        {
            ...
            let annotation = repo
                ...
                .and_then(|msg| msg.is_empty().not().then(|| bytes2string(msg).ok()).flatten());
...
pub fn get_tags_with_metadata(repo_path: &RepoPath) -> Result<Vec<TagWithMetadata>> {
    ...
    let mut tags: Vec<TagWithMetadata> = tags_with_commit_id
        ...
        .filter_map(|(tag, annotation, commit_id)| {
            unique_commit_infos.get(commit_id).map(|commit_info| TagWithMetadata {
...
mod tests {
    ...
    fn test_multitags() {
        ...
        let target = repo
            .find_object(repo.head().unwrap().target().unwrap(), Some(ObjectType::Commit))
            .unwrap();
