...
impl From<git2_hooks::HookResult> for HookResult {
    fn from(v: git2_hooks::HookResult) -> Self {
        match v {
            ...
            git2_hooks::HookResult::RunNotSuccessful { stdout, stderr, .. } => Self::NotOk(format!("{stdout}{stderr}")),
        }
    }
}
...
mod tests {
	...
	exit 1
        ...
        if let HookResult::NotOk(res) = res {
            assert_eq!(std::path::Path::new(res.trim_end()), std::path::Path::new(&workdir));
        } else {
            assert!(false);
        }
    }
