...
impl AsyncCommitFilterJob {
    ///
    pub fn new(repo_path: RepoPath, commits: Vec<CommitId>, filter: SharedCommitFilterFn) -> Self {
        Self {
                    ...
                    sync::repo(repo_path).ok().map(|repo| {
                        c.iter()
                            .filter_map(|(e, c)| {
                                ...
                                Self::update_progress(params, ProgressPercent::new(idx, total_amount));

                                (*self.filter)(&repo, c).ok().and_then(|res| res.then_some((*e, *c)))
                            })
                            .collect::<Vec<_>>()
                    })
                })
    ...
    fn update_progress(params: &RunParams<AsyncGitNotification, ProgressPercent>, new_progress: ProgressPercent) {
    ...
    fn run(&mut self, params: RunParams<Self::Notification, Self::Progress>) -> Result<Self::Notification> {
        if let Ok(mut state) = self.state.lock() {
            *state = state.take().map(|state| match state {
                JobState::Request { commits, repo_path } => self.run_request(&repo_path, commits, &params),
