...
impl AsyncLog {
    ///
    pub fn new(repo: RepoPath, sender: &Sender<AsyncGitNotification>, filter: Option<SharedCommitFilterFn>) -> Self {
        Self {
            repo,
    ...
    pub fn fetch(&mut self) -> Result<FetchStatus> {
        ...
        rayon_core::spawn(move || {
            ...
            Self::fetch_helper(&repo_path, &arc_current, &arc_background, &sender, filter).expect("failed to fetch");
    ...
    fn notify(sender: &Sender<AsyncGitNotification>) {
        sender.send(AsyncGitNotification::Log).expect("error sending");
    }
}
