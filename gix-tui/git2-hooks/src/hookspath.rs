...
impl HookPaths {
    ...
    pub fn new(repo: &Repository, other_paths: Option<&[&str]>, hook: &str) -> Result<Self> {
        ...
        if let Some(config_path) = Self::config_hook_path(repo)? {
            ...
            let hook = shellexpand::full(hook.as_os_str().to_str().ok_or(HooksError::PathToString)?)?;
    ...
    fn find_hook(repo: &Repository, other_paths: Option<&[&str]>, hook: &str) -> PathBuf {
        ...
        repo.path().to_path_buf().join(DEFAULT_HOOKS_PATH).join(hook)
    }
    ...
    pub fn run_hook(&self, args: &[&str]) -> Result<HookResult> {
        ...
        let output = Command::new(git_bash)
            ...
            .env("DUMMY_ENV_TO_FIX_WINDOWS_CMD_RUNS", "FixPathHandlingOnWindows")
            .output()?;
