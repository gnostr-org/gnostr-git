...
pub fn hooks_commit_msg(repo: &Repository, other_paths: Option<&[&str]>, msg: &mut String) -> Result<HookResult> {
...
exit 0
        ...
        create_hook_in_path(dbg!(custom_hooks_path.join(HOOK_PRE_COMMIT).as_path()), hook);
...
exit 1
        ";
            ...
            let custom_hooks_path = td.path().join(".myhooks");
            std::fs::create_dir(dbg!(&custom_hooks_path)).unwrap();
            create_hook_in_path(dbg!(custom_hooks_path.join(HOOK_PRE_COMMIT).as_path()), reject_hook);
        }
...
exit 0
        ...
        let mut msg = String::from("test");
        let res = hooks_prepare_commit_msg(&repo, None, PrepareCommitMsgSource::Message, &mut msg).unwrap();
...
exit 2
        ...
        let mut msg = String::from("test");
        let res =
            hooks_prepare_commit_msg(&repo, None, PrepareCommitMsgSource::Commit(git2::Oid::zero()), &mut msg).unwrap();
