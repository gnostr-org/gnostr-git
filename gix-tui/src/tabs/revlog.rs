...
use asyncgit::{
    ...
    sync::{self, filter_commit_by_search, CommitId, LogFilterSearch, LogFilterSearchOptions, RepoPathRef},
    AsyncBranchesJob, AsyncCommitFilterJob, AsyncGitNotification, AsyncLog, AsyncTags, CommitFilesParams, FetchStatus,
    ProgressPercent,
};
...
use crate::{
    components::{
        visibility_blocking, CommandBlocking, CommandInfo, CommitDetailsComponent, CommitList, Component,
        DrawableComponent, EventState, FileTreeOpen, InspectCommitOpen,
    },
...
impl Revlog {
    ...
    pub fn new(
        ...
        Self {
            repo: repo.clone(),
            queue: queue.clone(),
            commit_details: CommitDetailsComponent::new(repo, queue, sender, theme.clone(), key_config.clone()),
            list: CommitList::new(
                repo.clone(),
            ...
            git_log: AsyncLog::new(repo.borrow().clone(), sender, None),
                ...
                if search.is_pending() {
                    ...
                    *progress = search.progress();
                    ...
                    match search {
                        Ok(search) => {
                            self.list
                                .set_highlighting(Some(Rc::new(search.result.into_iter().collect::<IndexSet<_>>())));

                            self.search = LogSearch::Results(LogSearchResult {
    ...
    fn draw_search<B: Backend>(&self, f: &mut Frame<B>, area: Rect) {
        let (text, title) = match &self.search {
            LogSearch::Searching(_, options, progress) => (
                ...
                format!("({}%)", progress.map(|progress| progress.progress).unwrap_or_default()),
            ),
        ...
        if self.visible {
            ...
            if event_used.is_consumed() {
                ...
                if key_match(k, self.key_config.keys.enter) {
                    ...
                    return self.selected_commit().map_or(Ok(EventState::NotConsumed), |id| {
                        self.queue.push(InternalEvent::TagCommit(id));
                        Ok(EventState::Consumed)
                    });
                } else if key_match(k, self.key_config.keys.move_right) && self.commit_details.is_visible() {
                    self.inspect_commit();
                    return Ok(EventState::Consumed);
                    ...
                    return self.selected_commit().map_or(Ok(EventState::NotConsumed), |id| {
                        self.queue.push(InternalEvent::OpenPopup(StackablePopupOpen::FileTree(
                            FileTreeOpen::new(id),
                        )));
                        Ok(EventState::Consumed)
                    });
                    ...
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.log_reset_comit) {
                    return self.selected_commit().map_or(Ok(EventState::NotConsumed), |id| {
                        self.queue.push(InternalEvent::OpenResetPopup(id));
                        Ok(EventState::Consumed)
                    });
                } else if key_match(k, self.key_config.keys.log_reword_comit) {
                    return self.selected_commit().map_or(Ok(EventState::NotConsumed), |id| {
                    ...
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.compare_commits) && self.list.marked_count() > 0 {
                    if self.list.marked_count() == 1 {
                        // compare against head
                        self.queue
                            .push(InternalEvent::OpenPopup(StackablePopupOpen::CompareCommits(
                                InspectCommitOpen::new(self.list.marked()[0].1),
                            )));
                        return Ok(EventState::Consumed);
    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        ...
        out.push(
            CommandInfo::new(
                ...
                true,
                (self.visible && self.can_leave_search()) || force_all,
            )
        ...
        out.push(CommandInfo::new(
            ...
            (self.visible && self.list.marked_count() <= 1) || force_all,
        ));

        out.push(CommandInfo::new(
            strings::commands::compare_commits(&self.key_config),
            true,
            (self.visible && self.list.marked_count() == 2) || force_all,
        ));
