...
use asyncgit::{
    ...
    AsyncBranchesJob, AsyncDiff, AsyncGitNotification, AsyncStatus, DiffParams, DiffType, PushType, StatusItem,
    StatusParams,
};
...
use crate::{
    accessors,
    components::{
        command_pump, event_pump, visibility_blocking, ChangesComponent, CommandBlocking, CommandInfo, Component,
        DiffComponent, DrawableComponent, EventState, FileTreeItemKind,
    },
...
impl DrawableComponent for Status {
    fn draw<B: ratatui::backend::Backend>(&self, f: &mut ratatui::Frame<B>, rect: ratatui::layout::Rect) -> Result<()> {
...
impl Status {
    ...
    pub fn new(
        ...
        Self {
            ...
            git_status_workdir: AsyncStatus::new(repo_clone.clone(), sender.clone()),
            git_status_stage: AsyncStatus::new(repo_clone, sender.clone()),
    ...
    fn draw_branch_state<B: ratatui::backend::Backend>(
        ...
        if let Some(branch_name) = self.git_branch_name.last() {
            let ahead_behind = self.git_branch_state.as_ref().map_or_else(String::new, |state| {
                format!("\u{2191}{} \u{2193}{} ", state.ahead, state.behind,)
            });

            let w = Paragraph::new(format!("{ahead_behind}{{{branch_name}}}")).alignment(Alignment::Right);
    ...
    fn draw_repo_state<B: ratatui::backend::Backend>(&self, f: &mut ratatui::Frame<B>, r: ratatui::layout::Rect) {
    ...
    pub fn update_git(&mut self, ev: AsyncGitNotification) -> Result<()> {
        ...
        match ev {
            ...
            AsyncGitNotification::Push | AsyncGitNotification::Pull | AsyncGitNotification::CommitFiles => {
                self.branch_compare();
            }
            _ => (),
        }
    ...
    fn update_status(&mut self) -> Result<()> {
        ...
        if self.git_action_executed {
            ...
            if self.focus == Focus::WorkDir && workdir_status.items.is_empty() && !stage_status.items.is_empty() {
    ...
    pub fn update_diff(&mut self) -> Result<()> {
        if let Some((path, is_stage)) = self.selected_path() {
            let diff_type = if is_stage { DiffType::Stage } else { DiffType::WorkDir };
    ...
    fn request_diff(&mut self, diff_params: DiffParams, path: String, is_stage: bool) -> Result<(), anyhow::Error> {
    ...
    fn push(&self, force: bool) {
        if self.can_push() {
            if let Some(branch) = self.git_branch_name.last() {
                if force {
                    self.queue
                        .push(InternalEvent::ConfirmAction(Action::ForcePush(branch, force)));
                } else {
                    self.queue
    ...
    fn undo_last_commit(&self) {
        self.queue.push(InternalEvent::ConfirmAction(Action::UndoCommit));
    }

    fn branch_compare(&mut self) {
        self.git_branch_state = self
            .git_branch_name
            .last()
            .and_then(|branch| sync::branch_compare_upstream(&self.repo.borrow(), branch.as_str()).ok());
    }

    fn can_push(&self) -> bool {
        self.git_branch_state.as_ref().map_or(true, |state| state.ahead > 0) && self.has_remotes
    }
    ...
    pub fn abort_rebase(&self) {
        try_or_popup!(self, "abort rebase", sync::abort_pending_rebase(&self.repo.borrow()));
    }
    ...
    fn event(&mut self, ev: &crossterm::event::Event) -> Result<EventState> {
        if self.visible {
            ...
            if let Event::Key(k) = ev {
                ...
                } else if key_match(k, self.key_config.keys.toggle_workarea) && !self.is_focus_on_diff() {
                    self.switch_focus(self.focus.toggled_focus()).map(Into::into)
                    ...
                    && !self.index_wd.is_empty()
                {
                ...
                } else if key_match(k, self.key_config.keys.select_branch) && !self.is_focus_on_diff() {
                    self.queue.push(InternalEvent::SelectBranch);
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.force_push) && !self.is_focus_on_diff() && self.can_push() {
                    self.push(true);
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.push) && !self.is_focus_on_diff() {
                    self.push(false);
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.fetch) && !self.is_focus_on_diff() && self.can_pull() {
                    self.fetch();
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.pull) && !self.is_focus_on_diff() && self.can_pull() {
                    self.pull();
                    Ok(EventState::Consumed)
                    ...
                    if self.can_abort_merge() {
                        self.queue.push(InternalEvent::ConfirmAction(Action::AbortMerge));
                    } else if self.pending_rebase() {
                        self.queue.push(InternalEvent::ConfirmAction(Action::AbortRebase));
                    } else if self.pending_revert() {
                        self.queue.push(InternalEvent::ConfirmAction(Action::AbortRevert));
                    }
