...
use syntect::{
    highlighting::{FontStyle, HighlightState, Highlighter, RangedHighlightIterator, Style, ThemeSet},
...

        {
            ...
            for (number, line) in text.lines().enumerate() {
                ...
                syntax_lines.push(SyntaxLine {
                    items: iter.map(|(style, _, range)| (style, number, range)).collect(),
                });
...
impl AsyncJob for AsyncSyntaxJob {
    ...
    fn run(&mut self, params: RunParams<Self::Notification, Self::Progress>) -> asyncgit::Result<Self::Notification> {
        ...
        Ok(AsyncAppNotification::SyntaxHighlighting(SyntaxHighlightProgress::Done))
    }
}
