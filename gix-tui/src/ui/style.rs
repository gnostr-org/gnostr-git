...
impl Theme {
    ...
    pub fn tags(&self, selected: bool) -> Style {
        Style::default()
            ...
            .bg(if selected { self.selection_bg } else { Color::Reset })
    }
    ...
    pub fn diff_line(&self, typ: DiffLineType, selected: bool) -> Style {
        let style = match typ {
            ...
            DiffLineType::Header => Style::default().fg(self.disabled_fg).add_modifier(Modifier::BOLD),
            DiffLineType::None => Style::default().fg(if selected { self.command_fg } else { Color::Reset }),
        };
    ...
    pub fn log_marker(&self, selected: bool) -> Style {
        let mut style = Style::default().fg(self.commit_author).add_modifier(Modifier::BOLD);
    ...
    pub fn commit_hash_in_blame(&self, is_blamed_commit: bool) -> Style {
        if is_blamed_commit {
            Style::default().fg(self.commit_hash).add_modifier(Modifier::BOLD)
        } else {
    ...
    pub fn push_gauge(&self) -> Style {
        Style::default().fg(self.push_gauge_fg).bg(self.push_gauge_bg)
    }
