...
mod test {
    ...
    fn run_composer(which: Composer, text: &str, text_area_width: u16) -> (Vec<String>, Vec<u16>) {
        ...
        let mut styled = UnicodeSegmentation::graphemes(text, true).map(|g| StyledGrapheme { symbol: g, style });
        let mut composer: Box<dyn LineComposer> = match which {
            Composer::WordWrapper { trim } => Box::new(WordWrapper::new(&mut styled, text_area_width, trim)),
    ...
    fn line_composer_word_wrapper_nbsp() {
        ...
        let (word_wrapper_space, _) = run_composer(Composer::WordWrapper { trim: true }, &text_space, width);
