...
impl RepoWatcher {
    ...
    fn forwarder(
        receiver: &std::sync::mpsc::Receiver<Result<Vec<DebouncedEvent>, Vec<Error>>>,
        sender: &Sender<()>,
    ) -> Result<()> {
        loop {
...
fn create_watcher(
    timeout: Duration,
    tx: std::sync::mpsc::Sender<Result<Vec<DebouncedEvent>, Vec<Error>>>,
    workdir: &str,
) {
    scope_time!("create_watcher");

    let mut bouncer = new_debouncer(timeout, None, tx).expect("Watch create error");
    bouncer
        .watcher()
