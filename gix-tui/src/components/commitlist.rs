use std::{borrow::Cow, cell::Cell, cmp, collections::BTreeMap, convert::TryFrom, rc::Rc, time::Instant};

use anyhow::Result;
use asyncgit::sync::{self, checkout_commit, BranchDetails, BranchInfo, CommitId, RepoPathRef, Tags};
...
use crate::{
    components::{
        utils::string_width_align, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState, ScrollType,
    },
...
impl CommitList {
    ///
    pub fn new(repo: RepoPathRef, title: &str, theme: SharedTheme, queue: Queue, key_config: SharedKeyConfig) -> Self {
        Self {
            repo,
        ...
        let yank: Option<String> = match marked {
            ...
            [first, .., last] => {
                ...
                let yank = if marked_consecutive {
                    format!("{}^..{}", first.1.to_string(), last.1.to_string())
                } else {
                    marked.iter().map(|(_idx, commit)| commit.to_string()).join(" ")
                };
                Some(yank)
            }
        };
    ...
    pub fn set_highlighting(&mut self, highlighting: Option<Rc<IndexSet<CommitId>>>) {
        ...
        self.highlights = if highlighting.as_ref().map(|set| set.is_empty()).unwrap_or_default() {
            None
        } else {
            highlighting
        };
    ...
    fn move_selection_normal(&mut self, scroll: ScrollType) -> Result<bool> {
        ...
        let page_offset = usize::from(self.current_size.get().unwrap_or_default().1).saturating_sub(1);
    ...
    fn mark(&mut self) {
        if let Some(e) = self.selected_entry() {
            ...
            if self.is_marked(&id).unwrap_or_default() {
                ...
                self.marked.sort_unstable_by(|first, second| first.0.cmp(&second.0));
            }
        }
    }
    ...
    fn get_entry_to_add<'a>(
        ...
        let mut txt: Vec<Span> = Vec::with_capacity(ELEMENTS_PER_LINE + if marked.is_some() { 2 } else { 0 });
        ...
        if let Some(marked) = marked {
            txt.push(Span::styled(
                Cow::from(if marked { symbol::CHECKMARK } else { symbol::EMPTY_SPACE }),
        ...
        // commit msg
        txt.push(Span::styled(format!("{:message_width$}", &e.msg), style_msg));
    ...
    fn get_text(&self, height: usize, width: usize) -> Vec<Line> {
        ...
        for (idx, e) in self.items.iter().skip(self.scroll_top.get()).take(height).enumerate() {
            let tags = self
                .tags
                .as_ref()
            ...
            let marked = if any_marked { self.is_marked(&e.id) } else { None };
                    ...
                    self.local_branches.get(&e.id).map_or(true, |local_branch| {
                        local_branch.iter().any(|local_branch| {
                            let has_corresponding_local_branch = match &local_branch.details {
                                BranchDetails::Local(details) => details
                                    .upstream
                                    .as_ref()
    ...
    fn is_list_in_sync(&self) -> bool {
        self.items
            .index_offset_raw()
            .and_then(|index| self.items.iter().next().map(|item| item.id == self.commits[index]))
            .unwrap_or_default()
    }
...
impl DrawableComponent for CommitList {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
        ...
        self.scroll_top
            .set(calc_scroll_top(self.scroll_top.get(), height_in_lines, selection));
...
impl Component for CommitList {
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if let Event::Key(k) = ev {
            ...
            } else if key_match(k, self.key_config.keys.shift_up) || key_match(k, self.key_config.keys.home) {
                self.move_selection(ScrollType::Home)?
            } else if key_match(k, self.key_config.keys.shift_down) || key_match(k, self.key_config.keys.end) {
...
mod tests {
    ...
    fn test_string_width_align_unicode() {
        ...
        assert_eq!(string_width_align("Jon Grythe Stødle", 19), "Jon Grythe Stødle  ");
    }
}
