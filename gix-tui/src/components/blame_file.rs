...
use super::{
    utils, visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState, FileRevOpen,
    InspectCommitOpen,
};
use crate::{
...
impl Component for BlameFileComponent {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            out.push(CommandInfo::new(strings::commands::close_popup(&self.key_config), true, true).order(1));
            out.push(
                CommandInfo::new(
    ...
    fn event(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if self.is_visible() {
            if let Event::Key(key) = event {
                if key_match(key, self.key_config.keys.exit_popup) {
                ...
                } else if key_match(key, self.key_config.keys.shift_up) || key_match(key, self.key_config.keys.home) {
                    self.move_selection(ScrollType::Home);
                } else if key_match(key, self.key_config.keys.shift_down) || key_match(key, self.key_config.keys.end) {
                    ...
                    if let Some(filepath) = self.params.as_ref().map(|p| p.file_path.clone()) {
                        self.hide_stacked(true);
                        self.queue.push(InternalEvent::OpenPopup(StackablePopupOpen::FileRevlog(
...
impl BlameFileComponent {
    ...
    pub fn new(
        ...
        Self {
            ...
            theme,
            async_blame: AsyncBlame::new(repo.borrow().clone(), sender),
            queue: queue.clone(),
            visible: false,
            params: None,
            file_blame: None,
    ...
    fn hide_stacked(&mut self, stack: bool) {
        ...
        if stack {
            if let Some(request) = self.open_request.clone() {
                self.queue
                    .push(InternalEvent::PopupStackPush(StackablePopupOpen::BlameFile(
                        BlameFileOpen {
    ...
    fn update(&mut self) -> Result<()> {
        if self.is_visible() {
            if let Some(params) = &self.params {
                if let Some((previous_blame_params, last_file_blame)) = self.async_blame.last()? {
    ...
    fn get_title(&self) -> String {
        match (self.any_work_pending(), self.params.as_ref(), self.file_blame.as_ref()) {
            (true, Some(params), _) => {
                format!(
            ...
            (false, Some(params), None) => {
                format!("{} -- {} -- <no blame available>", self.title, params.file_path)
            }
    ...
    fn get_rows(&self, width: usize) -> Vec<Row> {
        self.file_blame.as_ref().map_or_else(Vec::new, |file_blame| {
            file_blame
                .lines
                .iter()
                .enumerate()
                .map(|(i, (blame_hunk, line))| {
                    self.get_line_blame(width, i, (blame_hunk.as_ref(), line.as_ref()), file_blame)
                })
                .collect()
        })
    }
    ...
    fn get_line_blame(
        ...
        cells.push(
            Cell::from(format!("{line_number:>line_number_width$}{VERTICAL}",)).style(self.theme.text(true, false)),
        );
        cells.push(Cell::from(tabs_to_spaces(String::from(line))).style(self.theme.text(true, false)));

        Row::new(cells)
    }

    fn get_metadata_for_line_blame(&self, width: usize, blame_hunk: Option<&BlameHunk>) -> Vec<Cell> {
        let commit_hash = blame_hunk.map_or_else(|| NO_COMMIT_ID.into(), |hunk| hunk.commit_id.get_short_string());
        ...
        let time = blame_hunk.map_or_else(String::new, |hunk| utils::time_to_string(hunk.time, true));
    ...
    fn move_selection(&mut self, scroll_type: ScrollType) -> bool {
        ...
        let new_selection = match scroll_type {
            ...
            ScrollType::PageUp => old_selection.saturating_sub(self.current_height.get().saturating_sub(2)),
    ...
    fn selected_commit(&self) -> Option<CommitId> {
        self.file_blame.as_ref().and_then(|file_blame| {
            ...
            let commit_id = table_state
                .selected()
                .and_then(|selected| file_blame.lines[selected].0.as_ref().map(|hunk| hunk.commit_id));
