...
use asyncgit::{
    sync::{
        ...
        checkout_branch, get_branches_info, BranchInfo, BranchType, CommitId, RepoPathRef, RepoState,
    },
...
use super::{
    utils::scroll_vertical::VerticalScroll, visibility_blocking, CommandBlocking, CommandInfo, Component,
    DrawableComponent, EventState, FuzzyFinderTarget, InspectCommitOpen,
};
use crate::{
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        ...
        if let Event::Key(e) = ev {
            ...
            if key_match(e, self.key_config.keys.enter) {
                try_or_popup!(self, "switch branch error:", self.switch_to_selected_branch());
                ...
                && self.valid_selection()
            {
            ...
            } else if key_match(e, self.key_config.keys.fetch) && !self.local && self.has_remotes {
                ...
                self.queue
                    .push(InternalEvent::OpenFuzzyFinder(branches, FuzzyFinderTarget::Branches));
            }
        }
...
impl BranchListComponent {
    pub fn new(repo: RepoPathRef, queue: Queue, theme: SharedTheme, key_config: SharedKeyConfig) -> Self {
        Self {
            branches: Vec::new(),
            local: true,
            has_remotes: false,
    ...
    fn selection_is_cur_branch(&self) -> bool {
        self.branches
            ...
            .filter(|(index, b)| {
                b.local_details()
                    .map(|details| details.is_head && *index == self.selection as usize)
                    .unwrap_or_default()
            })
            .count()
            > 0
    ...
    fn get_selected(&self) -> Option<CommitId> {
        self.branches.get(usize::from(self.selection)).map(|b| b.top_commit)
    }
    ...
    fn get_text(&self, theme: &SharedTheme, width_available: u16, height: usize) -> Text {
            ...
            .enumerate()
        {
            ...
            if commit_message.len() > commit_message_length {
                commit_message.unicode_truncate(commit_message_length.saturating_sub(THREE_DOTS_LENGTH));
            ...
            let upstream_tracking_str = match displaybranch.details {
                BranchDetails::Local(LocalBranch { has_upstream, .. }) if has_upstream => UPSTREAM_SYMBOL,
                BranchDetails::Remote(RemoteBranch { has_tracking, .. }) if has_tracking => TRACKING_SYMBOL,
                _ => EMPTY_SYMBOL,
            };
            ...
            txt.push(Line::from(vec![span_prefix, span_name, span_hash, span_msg]));
        }
    ...
    fn switch_to_selected_branch(&mut self) -> Result<()> {
        ...
        if self.local {
            checkout_branch(&self.repo.borrow(), &self.branches[self.selection as usize].name)?;
            self.hide();
        } else {
    ...
    fn draw_list<B: Backend>(&self, f: &mut Frame<B>, r: Rect) -> Result<()> {
        let height_in_lines = r.height as usize;
        self.current_height.set(height_in_lines.try_into()?);

        self.scroll
            .update(self.selection as usize, self.branches.len(), height_in_lines);

        f.render_widget(
            Paragraph::new(self.get_text(&self.theme, r.width, height_in_lines)).alignment(Alignment::Left),
            r,
        );
