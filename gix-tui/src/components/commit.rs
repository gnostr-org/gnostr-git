...
use asyncgit::{
    ...
    sync::{self, get_config_string, CommitId, HookResult, PrepareCommitMsgSource, RepoPathRef, RepoState},
...
use super::{
    textinput::TextInputComponent, visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent,
...
impl CommitComponent {
    ...
    fn draw_warnings<B: Backend>(&self, f: &mut Frame<B>) {
        let first_line = self.input.get_text().lines().next().map(str::len).unwrap_or_default();
    ...
    pub fn show_editor(&mut self, changes: Vec<StatusItem>) -> Result<()> {
        ...
        ExternalEditorComponent::open_file_in_editor(&self.repo.borrow(), &file_path)?;
    ...
    fn commit(&mut self) -> Result<()> {
        let gpgsign = get_config_string(&self.repo.borrow(), "commit.gpgsign")
            .ok()
            .flatten()
        ...
        if matches!(self.commit_with_msg(msg)?, CommitResult::ComitDone) {
            self.options.borrow_mut().add_commit_msg(self.input.get_text());
    ...
    fn commit_with_msg(&mut self, msg: String) -> Result<CommitResult> {
        ...
        if verify {
            ...
            if let HookResult::NotOk(e) = sync::hooks_pre_commit(&self.repo.borrow())? {
                ...
                self.queue
                    .push(InternalEvent::ShowErrorMsg(format!("pre-commit hook error:\n{e}")));
        ...
        if verify {
            ...
            if let HookResult::NotOk(e) = sync::hooks_commit_msg(&self.repo.borrow(), &mut msg)? {
                ...
                self.queue
                    .push(InternalEvent::ShowErrorMsg(format!("commit-msg hook error:\n{e}")));
                return Ok(CommitResult::Aborted);
            }
        }
        self.do_commit(&msg)?;
        ...
        if let HookResult::NotOk(e) = sync::hooks_post_commit(&self.repo.borrow())? {
            ...
            self.queue
                .push(InternalEvent::ShowErrorMsg(format!("post-commit hook error:\n{e}")));
        }
    ...
    pub fn open(&mut self, reword: Option<CommitId>) -> Result<()> {
        ...
        let (mode, msg_source) = if repo_state != RepoState::Clean && reword.is_some() {
            ...
            match repo_state {
                ...
                _ => {
                    self.commit_template = get_config_string(&self.repo.borrow(), "commit.template")
                        .map_err(|e| {
        ...
        if let HookResult::NotOk(e) = sync::hooks_prepare_commit_msg(&self.repo.borrow(), msg_source, &mut msg)? {
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.is_visible() {
            ...
            if let Event::Key(e) = ev {
                if key_match(e, self.key_config.keys.enter) && self.can_commit() {
                    ...
                    if let Some(msg) = self.options.borrow().commit_msg(self.commit_msg_history_idx) {
                        self.input.set_text(msg);
