...
use super::{
    visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState, TextInputComponent,
};
use crate::{
...
impl LogSearchPopupComponent {
    ...
    pub fn new(repo: RepoPathRef, queue: &Queue, theme: SharedTheme, key_config: SharedKeyConfig) -> Self {
        let mut find_text = TextInputComponent::new(theme.clone(), key_config.clone(), "", "search text", false);
        find_text.embed();
    ...
    fn set_mode(&mut self, mode: &PopupMode) {
        ...
        match mode {
            PopupMode::Search => {
                ...
                self.find_text.enabled(matches!(self.selection, Selection::EnterText));
            }
    ...
    fn execute_confirm(&mut self) {
        ...
        match self.mode {
            PopupMode::Search => {
                self.queue.push(InternalEvent::CommitSearch(LogFilterSearchOptions {
            ...
            PopupMode::JumpCommitSha => {
                ...
                self.queue.push(InternalEvent::SelectCommitInRevlog(commit_id));
            }
        }
    }
    ...
    fn get_text_options(&self) -> Vec<Line> {
        ...
        vec![
            Line::from(vec![Span::styled(
                ...
                self.theme.text(matches!(self.selection, Selection::FuzzyOption), false),
            )]),
            Line::from(vec![Span::styled(
                format!("[{x_opt_casesensitive}] case sensitive"),
                self.theme.text(matches!(self.selection, Selection::CaseOption), false),
            )]),
            Line::from(vec![Span::styled(
            ...
            Line::from(vec![Span::styled(
                ...
                self.theme
                    .text(matches!(self.selection, Selection::MessageBodySearch), false),
            )]),
            Line::from(vec![Span::styled(
    ...
    fn draw_search_mode<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
        ...
        f.render_widget(
            Block::default()
                ...
                .title(Span::styled(strings::POPUP_TITLE_LOG_SEARCH, self.theme.title(true))),
            area,
        );
    ...
    fn draw_commit_sha_mode<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
    ...
    fn event_commit_sha_mode(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if let Event::Key(key) = &event {
            if key_match(key, self.key_config.keys.exit_popup) {
                ...
                self.find_text.enabled(!self.find_text.get_text().trim().is_empty());
            }
        }
    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            if matches!(self.mode, PopupMode::Search) {
                out.push(CommandInfo::new(strings::commands::scroll_popup(&self.key_config), true, true).order(1));
                out.push(
                    CommandInfo::new(
                ...
                out.push(CommandInfo::new(strings::commands::find_commit_sha(&self.key_config), true, true).order(1));
            }
