...
impl PullComponent {
    ...
    pub fn update_git(&mut self, ev: AsyncGitNotification) {
        if self.is_visible() && ev == AsyncGitNotification::Pull {
            if let Err(error) = self.update() {
                ...
                self.hide();
                self.queue
                    .push(InternalEvent::ShowErrorMsg(format!("fetch failed:\n{error}")));
            }
        }
    }
    ...
    fn confirm_merge(&mut self, incoming: usize) {
        self.queue.push(InternalEvent::ConfirmAction(Action::PullMerge {
            incoming,
...
impl DrawableComponent for PullComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, rect: Rect) -> Result<()> {
        if self.visible {
            let (state, progress) = PushComponent::get_progress(&self.progress);
            ...
            f.render_widget(
                Gauge::default()
                    ...
                    .block(
                        Block::default()
                            .title(Span::styled(strings::PULL_POPUP_MSG, self.theme.title(true)))
                            .borders(Borders::ALL)
