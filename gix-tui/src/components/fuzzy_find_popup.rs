...
use super::{
    visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState, FuzzyFinderTarget,
...
impl FuzzyFindPopup {
    ///
    pub fn new(queue: &Queue, theme: SharedTheme, key_config: SharedKeyConfig) -> Self {
        let mut find_text = TextInputComponent::new(theme.clone(), key_config.clone(), "", "start typing..", false);
        find_text.embed();
    ...
    fn update_query(&mut self) {
        if self.find_text.get_text().is_empty() {
            self.set_query(None);
        } else if self.query.as_ref().map_or(true, |q| q != self.find_text.get_text()) {
    ...
    #[inline]
    fn draw_matches_list<B: Backend>(&self, f: &mut Frame<B>, mut area: Rect) {
        {
            ...
            let items = self
                ...
                .map(|(idx, indicies)| {
                    ...
                    let trim_length = self.contents[*idx].graphemes(true).count() - full_text.graphemes(true).count();
                    Line::from(
                        full_text
                            .graphemes(true)
                            .enumerate()
                            .map(|(c_idx, c)| {
                                Span::styled(
                                    ...
                                    self.theme.text(selected, indicies.contains(&(c_idx + trim_length))),
                                )
                            })
                            .collect::<Vec<_>>(),
                    )
        ...
        if self.is_visible() {
            ...
            f.render_widget(
                Block::default()
                    ...
                    .title(Span::styled(strings::POPUP_TITLE_FUZZY_FIND, self.theme.title(true))),
                area,
            );
    ...
    fn event(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if self.is_visible() {
            if let Event::Key(key) = event {
                if key_match(key, self.key_config.keys.exit_popup) || key_match(key, self.key_config.keys.enter) {
                    self.hide();
