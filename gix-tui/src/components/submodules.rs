...
use anyhow::Result;
use asyncgit::sync::{
    get_submodules, repo_dir, submodule_parent_info, update_submodule, RepoPathRef, SubmoduleInfo, SubmoduleParentInfo,
};
...
impl SubmodulesListComponent {
    ...
    fn can_open_submodule(&self) -> bool {
        self.selected_entry().map(|s| s.status.is_in_wd()).unwrap_or_default()
    }
    ...
    fn get_text(&self, theme: &SharedTheme, width_available: u16, height: usize) -> Text {
            ...
            .enumerate()
        {
            ...
            let span_hash = Span::styled(
                format!("{} ", submodule.head_id.unwrap_or_default().get_short_string()),
                theme.commit_hash(selected),
            );

            let span_name = Span::styled(format!("{module_path:name_length$} "), theme.text(true, selected));
    ...
    fn get_info_text(&self, theme: &SharedTheme) -> Text {
        self.selected_entry().map_or_else(Text::default, |submodule| {
            let span_title_path = Span::styled("Path:", theme.text(false, false));
            let span_path = Span::styled(submodule.path.to_string_lossy(), theme.text(true, false));

            let span_title_commit = Span::styled("Commit:", theme.text(false, false));
            let span_commit = Span::styled(submodule.id.unwrap_or_default().to_string(), theme.commit_hash(false));

            let span_title_url = Span::styled("Url:", theme.text(false, false));
            let span_url = Span::styled(submodule.url.clone().unwrap_or_default(), theme.text(true, false));

            let span_title_status = Span::styled("Status:", theme.text(false, false));
            let span_status = Span::styled(format!("{:?}", submodule.status), theme.text(true, false));
    ...
    fn get_local_info_text(&self, theme: &SharedTheme) -> Text {
        let mut spans = vec![
            ...
            Line::from(vec![Span::styled(self.repo_path.to_string(), theme.text(true, false))]),
    ...
    fn draw_list<B: Backend>(&self, f: &mut Frame<B>, r: Rect) -> Result<()> {
        ...
        self.scroll
            .update(self.selection as usize, self.submodules.len(), height_in_lines);
