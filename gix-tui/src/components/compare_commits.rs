...
use super::{
    command_pump, event_pump, visibility_blocking, CommandBlocking, CommandInfo, CommitDetailsComponent, Component,
    DiffComponent, DrawableComponent, EventState, InspectCommitOpen,
};
use crate::{
    accessors,
...
impl CompareCommitsComponent {
    ...
    pub fn new(
        ...
        Self {
            repo: repo.clone(),
            details: CommitDetailsComponent::new(repo, queue, sender, theme.clone(), key_config.clone()),
    ...
    fn get_ids(&self) -> Option<(CommitId, CommitId)> {
        ...
        self.open_request.as_ref().map(|open| open.commit_id).zip(other)
    }
    ...
    fn hide_stacked(&mut self, stack: bool) {
        ...
        if stack {
            if let Some(request) = self.open_request.clone() {
                self.queue
                    .push(InternalEvent::PopupStackPush(StackablePopupOpen::CompareCommits(
                        request,
                    )));
            }
        } else {
