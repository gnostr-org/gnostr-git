...
use crate::{
    ...
    strings,
    ui::{self, common_nav, style::SharedTheme, AsyncSyntaxJob, ParagraphState, ScrollPos, StatefulParagraph},
...
impl SyntaxTextComponent {
    ...
    pub fn load_file(&mut self, path: String, item: &TreeFile) {
        ...
        if !already_loaded {
            ...
            match sync::tree_file_content(&self.repo.borrow(), item) {
                ...
                Err(e) => {
                    self.current_file = Some((path, Either::Right(format!("error loading file: {e}"))));
                }
            }
        }
    }
    ...
    fn scroll(&self, nav: MoveSelection) -> bool {
        ...
        let new_scroll_pos = match nav {
            ...
            MoveSelection::Top => 0,
            MoveSelection::End => state.lines().saturating_sub(state.height().saturating_sub(2)),
            MoveSelection::PageUp => state.scroll().y.saturating_sub(state.height().saturating_sub(2)),
            MoveSelection::PageDown => state.scroll().y.saturating_add(state.height().saturating_sub(2)),
            _ => state.scroll().y,
        };
    ...
    fn set_scroll(&self, pos: u16) -> bool {
        ...
        let new_scroll_pos = pos.min(state.lines().saturating_sub(state.height().saturating_sub(2)));
...
impl DrawableComponent for SyntaxTextComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
        ...
        let content = StatefulParagraph::new(text).wrap(Wrap { trim: false }).block(
            Block::default()
                .title(title)
                .borders(Borders::ALL)
        ...
        if self.focused() {
            ui::draw_scrollbar(
                ...
                area,
                &self.theme,
                usize::from(state.lines().saturating_sub(state.height().saturating_sub(2))),
...
impl Component for SyntaxTextComponent {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.focused() || force_all {
            out.push(
                CommandInfo::new(strings::commands::scroll(&self.key_config), true, true).order(strings::order::NAV),
            );
        }
