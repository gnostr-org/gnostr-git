...
use crate::{
    components::{
        popup_paragraph, visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState,
    },
...
impl TextInputComponent {
    ...
    fn next_char_position(&self) -> Option<usize> {
        ...
        while index < self.msg.len() && !self.msg.is_char_boundary(index) {
            index += 1;
        }
        Some(index)
    }
    ...
    fn at_alphanumeric(&self, i: usize) -> bool {
        self.msg[i..].chars().next().map_or(false, char::is_alphanumeric)
    }
    ...
    fn next_word_position(&self) -> Option<usize> {
        ...
        let mut was_in_word = self.at_alphanumeric(self.cursor_position);
    ...
    fn get_draw_text(&self) -> Text {
        let style = self.theme.text(self.selected.unwrap_or(true), false);

        let mut txt = Text::default();
        // The portion of the text before the cursor is added
        // if the cursor is not at the first character.
        if self.cursor_position > 0 {
            let text_before_cursor = self.get_msg(0..self.cursor_position);
            let ends_in_nl = text_before_cursor.ends_with('\n');
            txt = text_append(txt, Text::styled(text_before_cursor, style));
            if ends_in_nl {
        ...
        let cursor_str = self
            ...
            .map_or(" ".to_owned(), |pos| self.get_msg(self.cursor_position..pos));
        ...
        if let Some(substitute) = cursor_highlighting.get(cursor_str.as_str()) {
            txt = text_append(
                txt,
                Text::styled(
                    substitute.to_owned(),
                    self.theme.text(false, false).add_modifier(Modifier::UNDERLINED),
                ),
            );
        } else {
            txt = text_append(txt, Text::styled(cursor_str, style.add_modifier(Modifier::UNDERLINED)));
        }
        ...
        if let Some(pos) = self.next_char_position() {
            if pos < self.msg.len() {
                txt = text_append(txt, Text::styled(self.get_msg(pos..self.msg.len()), style));
            }
        }
    ...
    fn draw_char_count<B: Backend>(&self, f: &mut Frame<B>, r: Rect) {
        if self.char_count > 0 {
            let w = Paragraph::new(format!("[{} chars]", self.char_count)).alignment(Alignment::Right);
        ...
        if self.visible {
            let txt = if self.msg.is_empty() {
                Text::styled(
                    ...
                    self.theme.text(self.selected.unwrap_or_default(), false),
                )
            } else {
                self.get_draw_text()
            };
            ...
            f.render_widget(
                popup_paragraph(self.title.as_str(), txt, &self.theme, true, !self.embed),
                area,
            );
...
impl Component for TextInputComponent {
    fn commands(&self, out: &mut Vec<CommandInfo>, _force_all: bool) -> CommandBlocking {
        out.push(CommandInfo::new(strings::commands::close_popup(&self.key_config), true, self.visible).order(1));
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.visible {
            if let Event::Key(e) = ev {
                ...
                let is_ctrl = e.modifiers.contains(KeyModifiers::CONTROL);
                ...
                match e.code {
                    ...
                    KeyCode::Delete if is_ctrl => {
                        if let Some(pos) = self.next_word_position() {
                            self.msg.replace_range(self.cursor_position..pos, "");
                            self.update_count();
                        }
                        return Ok(EventState::Consumed);
                    }
                    KeyCode::Backspace | KeyCode::Char('w') if is_ctrl => {
                        if let Some(pos) = self.previous_word_position() {
                            self.msg.replace_range(pos..self.cursor_position, "");
                            self.cursor_position = pos;
                            self.update_count();
                        }
                        return Ok(EventState::Consumed);
...
mod tests {
    ...
    #[test]
    fn test_smoke() {
        let mut comp = TextInputComponent::new(SharedTheme::default(), SharedKeyConfig::default(), "", "", false);
    ...
    fn text_cursor_initial_position() {
        ...
        let underlined = theme.text(true, false).add_modifier(Modifier::UNDERLINED);
        ...
        assert_eq!(get_style(&txt.lines[0].spans[0]), Some(&underlined));
    }
    ...
    fn test_cursor_second_position() {
        let mut comp = TextInputComponent::new(SharedTheme::default(), SharedKeyConfig::default(), "", "", false);
        let theme = SharedTheme::default();
        let underlined_whitespace = theme.text(false, false).add_modifier(Modifier::UNDERLINED);
        ...
        assert_eq!(get_text(&txt.lines[0].spans[0]), Some("a"));
        assert_eq!(get_style(&txt.lines[0].spans[0]), Some(&not_underlined));
        assert_eq!(get_text(&txt.lines[0].spans[1]), Some(symbol::WHITESPACE));
        assert_eq!(get_style(&txt.lines[0].spans[1]), Some(&underlined_whitespace));
    }
    ...
    fn test_visualize_newline() {
        ...
        let theme = SharedTheme::default();
        let underlined = theme.text(false, false).add_modifier(Modifier::UNDERLINED);
        ...
        assert_eq!(get_text(&txt.lines[0].spans[0]), Some("a"));
        assert_eq!(get_text(&txt.lines[0].spans[1]), Some("\u{21b5}"));
        assert_eq!(get_style(&txt.lines[0].spans[1]), Some(&underlined));
