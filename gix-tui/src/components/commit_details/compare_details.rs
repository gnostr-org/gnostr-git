...
impl CompareDetailsComponent {
    ///
    pub const fn new(repo: RepoPathRef, theme: SharedTheme, focused: bool) -> Self {
        Self {
            data: None,
            theme,
            focused,
    ...
    pub fn set_commits(&mut self, ids: Option<(CommitId, CommitId)>) {
        self.data = ids.and_then(|ids| {
            let c1 = sync::get_commit_details(&self.repo.borrow(), ids.0).ok();
            let c2 = sync::get_commit_details(&self.repo.borrow(), ids.1).ok();
    ...
    fn get_commit_text(&self, data: &CommitDetails) -> Vec<Line> {
        ...
        res.push(Line::from(vec![
            ...
            Span::styled(
                Cow::from(data.message.as_ref().map(|msg| msg.subject.clone()).unwrap_or_default()),
                self.theme.text(true, false),
            ),
        ]));
...
impl DrawableComponent for CompareDetailsComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, rect: Rect) -> Result<()> {
        ...
        if let Some(data) = &self.data {
            f.render_widget(
                dialog_paragraph(
                    &strings::commit::compare_details_info_title(true, data.0.short_hash()),
            ...
            f.render_widget(
                dialog_paragraph(
                    &strings::commit::compare_details_info_title(false, data.1.short_hash()),
