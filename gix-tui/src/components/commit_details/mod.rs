...
use super::{
    command_pump, event_pump, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState,
    StatusTreeComponent,
};
use crate::{
    accessors,
...
impl CommitDetailsComponent {
    ...
    pub fn new(
        ...
        Self {
            single_details: DetailsComponent::new(repo.clone(), theme.clone(), key_config.clone(), false),
            compare_details: CompareDetailsComponent::new(repo.clone(), theme.clone(), false),
            git_commit_files: AsyncCommitFiles::new(repo.borrow().clone(), sender),
            file_tree: StatusTreeComponent::new("", false, Some(queue.clone()), theme, key_config.clone()),
            visible: false,
            commit: None,
            key_config,
        }
    ...
    ///
    pub fn set_commits(&mut self, params: Option<CommitFilesParams>, tags: &Option<CommitTags>) -> Result<()> {
        if params.is_none() {
        ...
        if let Some(id) = params {
            ...
            if let Some(other) = id.other {
                self.compare_details.set_commits(Some((id.id, other)));
            } else {
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        ...
        if self.focused() {
            if let Event::Key(e) = ev {
                ...
                } else if key_match(e, self.key_config.keys.move_up) && self.file_tree.focused() && !self.is_compare() {
