use std::{borrow::Cow, cell::Cell, clone::Clone};
...
impl DetailsComponent {
    ///
    pub const fn new(repo: RepoPathRef, theme: SharedTheme, key_config: SharedKeyConfig, focused: bool) -> Self {
        Self {
            repo,
            data: None,
            tags: Vec::new(),
    ...
    fn get_text_info(&self) -> Vec<Line> {
        self.data.as_ref().map_or_else(Vec::new, |data| {
            ...
            res.push(Line::from(vec![
                Span::styled(Cow::from(strings::commit::details_sha()), self.theme.text(false, false)),
            ...
            if !self.tags.is_empty() {
                ...
                res.push(Line::from(
                    itertools::Itertools::intersperse(
                        self.tags
                            .iter()
                            .map(|tag| Span::styled(Cow::from(&tag.name), self.theme.text(true, false))),
...
impl DrawableComponent for DetailsComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, rect: Rect) -> Result<()> {
        ...
        self.scroll.update_no_selection(number_of_lines, usize::from(height));
...
mod tests {
    ...
    fn get_wrapped_lines(message: &CommitMessage, width: usize) -> Vec<Cow<'_, str>> {
        let (wrapped_title, wrapped_message) = DetailsComponent::wrap_commit_details(message, width);
    ...
    fn test_textwrap() {
        ...
        assert_eq!(get_wrapped_lines(&message_with_newline, 7), vec!["Commit", "message"]);
        assert_eq!(get_wrapped_lines(&message_with_newline, 14), vec!["Commit message"]);
        assert_eq!(get_wrapped_lines(&message, 0), vec!["Commit", "message"]);
