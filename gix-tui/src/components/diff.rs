...
impl DiffComponent {
    ...
    fn can_scroll(&self) -> bool {
        self.diff.as_ref().map(|diff| diff.lines > 1).unwrap_or_default()
    }
    ///
    ...
    pub fn update(&mut self, path: String, is_stage: bool, diff: FileDiff) {
        ...
        if self.current.hash != hash {
            ...
            self.current = Current { path, is_stage, hash };
    ...
    fn max_scroll_right(&self) -> usize {
        self.longest_line.saturating_sub(self.current_size.get().0.into())
    }
    ...
    fn get_text(&self, width: u16, height: u16) -> Vec<Line> {
        if let Some(diff) = &self.diff {
            return if diff.hunks.is_empty() {
                ...
                for (i, hunk) in diff.hunks.iter().enumerate() {
                    let hunk_selected = self.focused() && self.selected_hunk.map_or(false, |s| s == i);
    ...
    fn reset_hunk(&self) {
        if let Some(diff) = &self.diff {
            if let Some(hunk) = self.selected_hunk {
                ...
                self.queue.push(InternalEvent::ConfirmAction(Action::ResetHunk(
    ...
    fn reset_lines(&self) {
        self.queue.push(InternalEvent::ConfirmAction(Action::ResetLines(
            self.current.path.clone(),
            self.selected_lines(),
        )));
    }
        ...
        self.diff
            ...
            .map(|diff| {
                diff.hunks
                    ...
                    .enumerate()
                    .filter_map(|(i, line)| {
                        let is_add_or_delete =
                            line.line_type == DiffLineType::Add || line.line_type == DiffLineType::Delete;
    ...
    fn reset_untracked(&self) {
        self.queue.push(InternalEvent::ConfirmAction(Action::Reset(ResetItem {
...
impl DrawableComponent for DiffComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, r: Rect) -> Result<()> {
        ...
        let title = format!("{}{}", strings::title_diff(&self.key_config), self.current.path);
        ...
        f.render_widget(
            Paragraph::new(txt).block(
                Block::default()
                    .title(Span::styled(title.as_str(), self.theme.title(self.focused())))
                    .borders(Borders::ALL)
        ...
        if self.focused() {
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.move_down) {
                    ...
                    self.horizontal_scroll.move_right(HorizontalScrollType::Right);
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.move_left) {
                    self.horizontal_scroll.move_right(HorizontalScrollType::Left);
                    Ok(EventState::Consumed)
                    ...
                    self.diff_hunk_move_up_down(-1);
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.stage_unstage_item) && !self.is_immutable {
                    try_or_popup!(self, "hunk error:", self.stage_unstage_hunk());

                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.status_reset_item) && !self.is_immutable && !self.is_stage()
                {
                    if let Some(diff) = &self.diff {
                        if diff.untracked {
                    ...
                    self.stage_lines();
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.diff_reset_lines) && !self.is_immutable && !self.is_stage()
                {
                    if let Some(diff) = &self.diff {
...
mod tests {
    ...
    fn test_line_break() {
...

        {
            ...
            assert_eq!(
                DiffComponent::get_line_to_add(4, &diff_line, false, false, false, &default_theme, 0)
                    .spans
                    .last()
                    .unwrap(),
                &Span::styled(Cow::from("¶\n"), default_theme.diff_line(diff_line.line_type, false))
            );
        }
