...
impl FileTreeItem {
    fn new_file(item: &StatusItem) -> Result<Self> {
        ...
        let name = item_path.file_name().map(OsStr::to_string_lossy).map(|x| x.to_string());
...
mod tests {
    ...
    fn test_indent() {
        ...
        let mut res = list.items.iter().map(|i| (i.info.indent, i.info.path.as_str()));
    ...
    fn test_folder_dup() {
        ...
        assert_eq!(
            res,
            vec![String::from("a"), items[0].path.clone(), items[1].path.clone()]
        );
    }
