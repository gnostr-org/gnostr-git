...
impl ChangesComponent {
    ...
    pub fn new(
        ...
        Self {
            files: StatusTreeComponent::new(title, focus, Some(queue.clone()), theme, key_config.clone()),
            is_working_dir,
            queue,
            key_config,
            options,
        ...
        if let Some(tree_item) = self.selection() {
            if self.is_working_dir {
                if let FileTreeItemKind::File(i) = tree_item.kind {
                    ...
                    sync::stage_add_all(&self.repo.borrow(), tree_item.info.full_path.as_str(), config)?;
                }
                ...
                if sync::is_workdir_clean(&self.repo.borrow(), self.options.borrow().status_show_untracked())? {
    ...
    fn dispatch_reset_workdir(&mut self) -> bool {
        if let Some(tree_item) = self.selection() {
            let is_folder = matches!(tree_item.kind, FileTreeItemKind::Path(_));
        ...
        if self.focused() {
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.stage_unstage_item) {
                    ...
                    self.queue.push(InternalEvent::StatusLastFileMoved);
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.status_reset_item) && self.is_working_dir {
