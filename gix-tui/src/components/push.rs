...
impl PushComponent {
    ...
    ///
    pub fn push(&mut self, branch: String, push_type: PushType, force: bool, delete: bool) -> Result<()> {
        self.branch = branch;
            ...
            if let Event::Key(e) = ev {
                if self.input_cred.is_visible() {
                    ...
                    if self.input_cred.get_cred().is_complete() || !self.input_cred.is_visible() {
                        self.push_to_remote(Some(self.input_cred.get_cred().clone()), self.modifier.force())?;
