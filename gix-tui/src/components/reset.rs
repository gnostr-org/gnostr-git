...
use ratatui::{backend::Backend, layout::Rect, text::Text, widgets::Clear, Frame};
...
impl DrawableComponent for ConfirmComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, _rect: Rect) -> Result<()> {
        if self.visible {
    ...
    fn get_text(&self) -> (String, String) {
        if let Some(ref a) = self.target {
            return match a {
                ...
                Action::AbortRebase => (strings::confirm_title_abortrebase(), strings::confirm_msg_abortrebase()),
                Action::AbortRevert => (
                    strings::confirm_title_abortrevert(),
                    strings::confirm_msg_revertchanges(),
                ),
                Action::UndoCommit => (strings::confirm_title_undo_commit(), strings::confirm_msg_undo_commit()),
            };
        }
