use std::{borrow::Cow, collections::BTreeSet, convert::From, fmt::Write, path::Path};
...
use super::{
    utils::scroll_vertical::VerticalScroll, BlameFileOpen, CommandBlocking, CommandInfo, Component, DrawableComponent,
...
impl RevisionFilesComponent {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_file: SyntaxTextComponent::new(repo.clone(), sender, key_config.clone(), theme.clone()),
    ...
    fn tree_item_to_span<'a>(item: &'a FileTreeItem, theme: &SharedTheme, width: usize, selected: bool) -> Span<'a> {
    ...
    fn blame(&self) -> bool {
        self.selected_file_path().map_or(false, |path| {
            self.queue
                .push(InternalEvent::OpenPopup(StackablePopupOpen::BlameFile(BlameFileOpen {
                    file_path: path,
    ...
    fn selected_file_path_with_prefix(&self) -> Option<String> {
        self.tree.selected_file().map(|file| file.full_path_str().to_string())
    }

    fn selected_file_path(&self) -> Option<String> {
        self.tree
            .selected_file()
            .map(|file| file.full_path_str().strip_prefix("./").unwrap_or_default().to_string())
    }
            ...
            if let Some(files) = &self.files {
                ...
                if let Some(item) = files.iter().find(|f| f.path == path) {
                    if let Ok(path) = path.strip_prefix("./") {
                        return self.current_file.load_file(path.to_string_lossy().to_string(), item);
                    }
                }
    ...
    fn draw_tree<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
        ...
        self.tree.visual_selection().map_or_else(
            ...
            |selection| {
                self.scroll.update(selection.index, selection.count, tree_height);
            },
        );
        ...
        let items = self
            ...
            .map(|(item, selected)| Self::tree_item_to_span(item, &self.theme, tree_width, selected));
    ...
    fn title_within(&self, tree_width: usize) -> Result<String> {
        ...
        if let Some(message) = message {
            ...
            let available = tree_width
                ...
                .saturating_sub(2 /* frame end corners */ + 1 /* space */ + 2 /* square brackets */);

            if message.width() <= available {
                write!(title, " [{message}]")?;
            } else if available > 1 {
                write!(title, " [{}{}]", message.unicode_truncate(available - 1).0, ELLIPSIS)?;
            } else {
                title.push(ELLIPSIS);
            }
        }
        ...
        if let Event::Key(key) = event {
            ...
            if is_tree_focused && tree_nav(&mut self.tree, &self.key_config, key) {
                ...
                if let Some(file) = self.selected_file_path() {
                    try_or_popup!(self, strings::POPUP_FAIL_COPY, crate::clipboard::copy_string(&file));
                }
                return Ok(EventState::Consumed);
...
fn tree_nav_cmds(tree: &FileTree, key_config: &SharedKeyConfig, out: &mut Vec<CommandInfo>) {
    out.push(CommandInfo::new(strings::commands::navigate_tree(key_config), !tree.is_empty(), true).order(order::NAV));
}

//TODO: reuse for other tree usages
fn tree_nav(tree: &mut FileTree, key_config: &SharedKeyConfig, key: &crossterm::event::KeyEvent) -> bool {
