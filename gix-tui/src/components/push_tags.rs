...
use crate::{
    components::{
        cred::CredComponent, visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent,
        EventState,
    },
...
impl PushTagsComponent {
    ...
    pub fn new(
        ...
        Self {
            ...
            pending: false,
            visible: false,
            git_push: AsyncPushTags::new(repo.borrow().clone(), sender),
            progress: None,
            input_cred: CredComponent::new(theme.clone(), key_config.clone()),
            theme,
            key_config,
        }
    }
    ...
    pub fn push_tags(&mut self) -> Result<()> {
        ...
        if need_username_password(&self.repo.borrow())? {
            let cred =
                extract_username_password(&self.repo.borrow()).unwrap_or_else(|_| BasicAuthCredential::new(None, None));
            if cred.is_complete() {
    ...
    fn update(&mut self) -> Result<()> {
        ...
        if !self.pending {
            if let Some(err) = self.git_push.last_result()? {
                self.queue
                    .push(InternalEvent::ShowErrorMsg(format!("push tags failed:\n{err}")));
            }
            self.hide();
        }
    ...
    pub fn get_progress(progress: &Option<PushTagsProgress>) -> (String, u8) {
        progress
            ...
            .map_or((strings::PUSH_POPUP_PROGRESS_NONE.into(), 0), |progress| {
                (Self::progress_state_name(progress), progress.progress().progress)
            })
    }
        ...
        if self.visible {
            ...
            f.render_widget(
                Gauge::default()
                    ...
                    .block(
                        Block::default()
                            .title(Span::styled(strings::PUSH_TAGS_POPUP_MSG, self.theme.title(true)))
                            .borders(Borders::ALL)
