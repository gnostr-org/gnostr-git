    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::tag_commit_confirm_msg(&self.key_config),
                self.is_valid_tag(),
                true,
            ));
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.is_visible() {
            ...
            if let Event::Key(e) = ev {
                if key_match(e, self.key_config.keys.enter) && self.is_valid_tag() {
                    ...
                    self.input.clear();
                    self.input.set_title(strings::tag_popup_annotation_title(&tag_name));
                    self.input.set_default_msg(strings::tag_popup_annotation_msg());
...
impl TagCommitComponent {
    ...
    pub fn tag(&mut self) -> Result<()> {
        ...
        if let Some(commit_id) = self.commit_id {
            let result = sync::tag_commit(&self.repo.borrow(), &commit_id, &tag_name, tag_annotation.as_deref());
            match result {
                Ok(_) => {
                    self.input.clear();
                    self.hide();
