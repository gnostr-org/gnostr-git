...
use crate::{
    components::{
        visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState, InputType,
        TextInputComponent,
    },
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.visible {
            if let Event::Key(e) = ev {
                ...
                if self.input_username.event(ev)?.is_consumed() || self.input_password.event(ev)?.is_consumed() {
                    return Ok(EventState::Consumed);
                } else if key_match(e, self.key_config.keys.enter) {
                    if self.input_username.is_visible() {
                        self.cred = BasicAuthCredential::new(Some(self.input_username.get_text().to_string()), None);
