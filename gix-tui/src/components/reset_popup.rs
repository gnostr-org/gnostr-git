...
impl ResetPopupComponent {
    ///
    pub fn new(queue: &Queue, repo: &RepoPathRef, theme: SharedTheme, key_config: SharedKeyConfig) -> Self {
        Self {
            queue: queue.clone(),
    ...
    fn reset(&mut self) {
        if let Some(id) = self.commit {
            try_or_popup!(self, "reset:", asyncgit::sync::reset_repo(&self.repo, id, self.kind));
        }
...
impl Component for ResetPopupComponent {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            out.push(CommandInfo::new(strings::commands::reset_commit(&self.key_config), true, true).order(1));

            out.push(CommandInfo::new(strings::commands::reset_type(&self.key_config), true, true).order(1));
        }
