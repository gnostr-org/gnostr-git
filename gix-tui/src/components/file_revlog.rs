...
use super::{utils::logitems::ItemBatch, visibility_blocking, BlameFileOpen, InspectCommitOpen};
use crate::{
    components::{
        event_pump, CommandBlocking, CommandInfo, Component, DiffComponent, DrawableComponent, EventState, ScrollType,
    },
...
impl FileRevlogComponent {
    ...
    pub fn new(
        ...
        Self {
            ...
            git_log: None,
            git_diff: AsyncDiff::new(repo_path.borrow().clone(), sender),
            visible: false,
                ...
                if let Some(open_request) = &self.open_request {
                    ...
                    if let Some((params, last)) = self.git_diff.last()? {
                        if params == diff_params {
                            self.diff.update(open_request.file_path.to_string(), false, last);
    ...
    fn set_selection(&mut self, selection: usize) {
        let height_in_items = (self.current_height.get().saturating_sub(2)) / 2;
    ...
    fn draw_revlog<B: Backend>(&self, f: &mut Frame<B>, area: Rect) {
        ...
        let mut adjusted_table_state = TableState::default()
            ...
            .with_offset(table_state.offset().saturating_sub(self.items.index_offset()));
    ...
    fn hide_stacked(&mut self, stack: bool) {
        ...
        if stack {
            if let Some(open_request) = self.open_request.clone() {
                self.queue
                    .push(InternalEvent::PopupStackPush(StackablePopupOpen::FileRevlog(
                        FileRevOpen {
...
impl DrawableComponent for FileRevlogComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, area: Rect) -> Result<()> {
        if self.visible {
            let percentages = if self.diff.focused() { (0, 100) } else { (50, 50) };
