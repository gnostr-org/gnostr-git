...
use crate::{
    components::{visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState},
...
impl ExternalEditorComponent {
    ...
    pub fn open_file_in_editor(repo: &RepoPath, path: &Path) -> Result<()> {
        ...
        let first_char = *echars
            .peek()
            .ok_or_else(|| anyhow!("editor env variable found empty: {}", environment_options.join(" or ")))?;
