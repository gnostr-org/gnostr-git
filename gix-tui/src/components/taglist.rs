...
use super::{utils, visibility_blocking, CommandBlocking, CommandInfo, Component, DrawableComponent, EventState};
use crate::{
...
impl DrawableComponent for TagListComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, rect: Rect) -> Result<()> {
        if self.visible {
            ...
            let tag_name_width = self
                .tags
                .as_ref()
                .map_or(0, |tags| tags.iter().fold(0, |acc, tag| acc.max(tag.name.len())));
    ...
    fn event(&mut self, event: &Event) -> Result<EventState> {
        if self.visible {
            if let Event::Key(key) = event {
                ...
                } else if key_match(key, self.key_config.keys.move_right) && self.can_show_annotation() {
                    self.show_annotation();
                } else if key_match(key, self.key_config.keys.delete_tag) {
                    return self.selected_tag().map_or(Ok(EventState::NotConsumed), |tag| {
                        self.queue
                            .push(InternalEvent::ConfirmAction(Action::DeleteTag(tag.name.clone())));
                        Ok(EventState::Consumed)
                    });
                } else if key_match(key, self.key_config.keys.select_tag) {
                    return self.selected_tag().map_or(Ok(EventState::NotConsumed), |tag| {
                        self.queue.push(InternalEvent::SelectCommitInRevlog(tag.commit_id));
                        Ok(EventState::Consumed)
                    });
...
impl TagListComponent {
    ...
    fn move_selection(&mut self, scroll_type: ScrollType) -> bool {
        ...
        let new_selection = match scroll_type {
            ...
            ScrollType::PageUp => old_selection.saturating_sub(self.current_height.get().saturating_sub(1)),
    ...
    fn show_annotation(&self) {
        if let Some(tag) = self.selected_tag() {
            if let Some(annotation) = &tag.annotation {
                self.queue.push(InternalEvent::ShowInfoMsg(annotation.clone()));
            }
        }
    }

    fn can_show_annotation(&self) -> bool {
        self.selected_tag().and_then(|t| t.annotation.as_ref()).is_some()
    }
    ...
    fn get_rows(&self) -> Vec<Row> {
        self.tags
            .as_ref()
            .map_or_else(Vec::new, |tags| tags.iter().map(|tag| self.get_row(tag)).collect())
    }
    ...
    fn get_row(&self, tag: &TagWithMetadata) -> Row {
        ...
        let is_tag_missing_on_remote = self.missing_remote_tags.as_ref().map_or(false, |missing_remote_tags| {
        ...
        let cells: Vec<Cell> = vec![
            ...
            Cell::from(utils::time_to_string(tag.time, true)).style(self.theme.commit_time(false)),
    ...
    fn selected_tag(&self) -> Option<&TagWithMetadata> {
        self.tags.as_ref().and_then(|tags| {
            ...
            let tag = table_state.selected().and_then(|selected| tags.get(selected));
