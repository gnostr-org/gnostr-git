...
impl StatusTreeComponent {
    ...
    pub fn is_file_seleted(&self) -> bool {
        self.tree.selected_item().map_or(false, |item| match item.kind {
    ...
    fn item_to_text<'b>(
        ...
        match file_item_kind {
            FileTreeItemKind::File(status_item) => {
                ...
                let txt = if selected {
                    format!("{} {}{:w$}", status_char, indent_str, file, w = width as usize)
                } else {
                    format!("{status_char} {indent_str}{file}")
                };

                Some(Span::styled(Cow::from(txt), theme.item(status_item.status, selected)))
            }
            ...
            FileTreeItemKind::Path(path_collapsed) => {
                ...
                let txt = if selected {
                    format!("  {}{}{:w$}", indent_str, collapse_char, string, w = width as usize)
                } else {
    ...
    fn build_vec_text_draw_info_for_drawing(&self) -> (Vec<TextDrawInfo>, usize, usize) {
        ...
        (vec_draw_text_info, selection_offset, selection_offset_visible)
    }
        ...
        if let Some(item) = self.selection() {
            if crate::clipboard::copy_string(&item.info.full_path).is_err() {
                if let Some(queue) = &self.queue {
                    queue.push(InternalEvent::ShowErrorMsg(strings::POPUP_FAIL_COPY.to_string()));
                }
            }
        }
    }
...
impl DrawableComponent for StatusTreeComponent {
    fn draw<B: Backend>(&self, f: &mut Frame<B>, r: Rect) -> Result<()> {
        ...
        if self.pending {
            ...
            ui::draw_list(f, r, self.title.as_str(), items.into_iter(), self.focused, &self.theme);
        } else {
            ...
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.blame) {
                    if let Some(status_item) = self.selection_file() {
                        self.hide();
                        if let Some(queue) = &self.queue {
                            queue.push(InternalEvent::OpenPopup(StackablePopupOpen::BlameFile(BlameFileOpen {
                    ...
                    if let Some(status_item) = self.selection_file() {
                        self.hide();
                        if let Some(queue) = &self.queue {
                            queue.push(InternalEvent::OpenPopup(StackablePopupOpen::FileRevlog(
                    ...
                    if let Some(status_item) = self.selection_file() {
                        if let Some(queue) = &self.queue {
                            queue.push(InternalEvent::OpenExternalEditor(Some(status_item.path)));
                        }
                    }
                    Ok(EventState::Consumed)
                ...
                } else if key_match(e, self.key_config.keys.end) || key_match(e, self.key_config.keys.shift_down) {
...
mod tests {
    ...
    fn test_correct_scroll_position() {
        ...
        // set up file tree
        let mut ftc = StatusTreeComponent::new("title", true, None, SharedTheme::default(), SharedKeyConfig::default());
        ftc.update(&items).expect("Updating FileTreeComponent failed");
        ...
        ftc.draw(&mut frame, Rect::new(0, 0, 10, 5)).expect("Draw failed");
