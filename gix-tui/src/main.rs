...
fn run_app(
    ...
    let mut app = App::new(RefCell::new(repo), &tx_git, &tx_app, input.clone(), theme, key_config)?;
    ...
    loop {
        let event = if first_update {
            ...
            select_event(&rx_input, &rx_git, &rx_app, &rx_ticker, &rx_watcher, &spinner_ticker)?
        };

        {
            ...
            match event {
                ...
                QueueEvent::AsyncEvent(ev) => {
                    if !matches!(ev, AsyncNotification::Git(AsyncGitNotification::FinishUnchanged)) {
                        app.update_async(ev)?;
                    }
                }
