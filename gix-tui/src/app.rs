...
use crate::{
    ...
    cmdbar::CommandBar,
    components::{
        command_pump, event_pump, AppOption, BlameFileComponent, BranchListComponent, CommandInfo, CommitComponent,
        CompareCommitsComponent, Component, ConfirmComponent, CreateBranchComponent, DrawableComponent,
        ExternalEditorComponent, FetchComponent, FileRevlogComponent, FuzzyFindPopup, FuzzyFinderTarget, HelpComponent,
        InspectCommitComponent, LogSearchPopupComponent, MsgComponent, OptionsPopupComponent, PullComponent,
        PushComponent, PushTagsComponent, RenameBranchComponent, ResetPopupComponent, RevisionFilesPopup,
        StashMsgComponent, SubmodulesListComponent, TagCommitComponent, TagListComponent,
    },
...
impl App {
    ...
    pub fn new(
        ...
        let mut app = Self {
            input,
            reset: ConfirmComponent::new(queue.clone(), theme.clone(), key_config.clone()),
            ...
            stashmsg_popup: StashMsgComponent::new(repo.clone(), queue.clone(), theme.clone(), key_config.clone()),
            ...
            external_editor_popup: ExternalEditorComponent::new(theme.clone(), key_config.clone()),
            push_popup: PushComponent::new(&repo, &queue, sender, theme.clone(), key_config.clone()),
            push_tags_popup: PushTagsComponent::new(&repo, &queue, sender, theme.clone(), key_config.clone()),
            reset_popup: ResetPopupComponent::new(&queue, &repo, theme.clone(), key_config.clone()),
            pull_popup: PullComponent::new(&repo, &queue, sender, theme.clone(), key_config.clone()),
            fetch_popup: FetchComponent::new(repo.clone(), &queue, sender, theme.clone(), key_config.clone()),
            tag_commit_popup: TagCommitComponent::new(repo.clone(), queue.clone(), theme.clone(), key_config.clone()),
            ...
            tags_popup: TagListComponent::new(repo.clone(), &queue, sender, theme.clone(), key_config.clone()),
            options_popup: OptionsPopupComponent::new(&queue, theme.clone(), key_config.clone(), options.clone()),
            submodule_popup: SubmodulesListComponent::new(repo.clone(), &queue, theme.clone(), key_config.clone()),
            log_search_popup: LogSearchPopupComponent::new(repo.clone(), &queue, theme.clone(), key_config.clone()),
            fuzzy_find_popup: FuzzyFindPopup::new(&queue, theme.clone(), key_config.clone()),
            do_quit: QuitState::None,
            cmdbar: RefCell::new(CommandBar::new(theme.clone(), key_config.clone())),
            help: HelpComponent::new(theme.clone(), key_config.clone()),
            msg: MsgComponent::new(theme.clone(), key_config.clone()),
            revlog: Revlog::new(&repo, &queue, sender, theme.clone(), key_config.clone()),
            ...
            stashing_tab: Stashing::new(&repo, sender, &queue, theme.clone(), key_config.clone()),
            stashlist_tab: StashList::new(repo.clone(), &queue, theme.clone(), key_config.clone()),
        ...
        if let InputEvent::Input(ev) = ev {
            ...
            if matches!(polling_state, InputState::Paused) {
                let result = if let Some(path) = self.file_to_open.take() {
                    ExternalEditorComponent::open_file_in_editor(&self.repo.borrow(), Path::new(&path))
                } else {
...
impl App {
        ...
        match ev {
            ...
            InternalEvent::OptionSwitched(o) => {
                match o {
                    ...
                    AppOption::DiffContextLines | AppOption::DiffIgnoreWhitespaces | AppOption::DiffInterhunkLines => {
                        self.status_tab.update_diff()?;
                    }
                }
            ...
            InternalEvent::FuzzyFinderChanged(idx, content, target) => {
                match target {
                    FuzzyFinderTarget::Branches => self.select_branch_popup.branch_finder_update(idx)?,
                    FuzzyFinderTarget::Files => {
                        self.files_tab.file_finder_update(&PathBuf::from(content.clone()));
                        self.revision_files_popup.file_finder_update(&PathBuf::from(content));
                    }
                }
            ...
            InternalEvent::OpenRepo { path } => {
                let submodule_repo_path = RepoPath::Path(Path::new(&repo_work_dir(&self.repo.borrow())?).join(path));
    ...
    fn process_confirmed_action(&mut self, action: Action, flags: &mut NeedsUpdate) -> Result<()> {
        match action {
            ...
            Action::StashDrop(_) | Action::StashPop(_) => {
                if let Err(e) = self.stashlist_tab.action_confirmed(&self.repo.borrow(), &action) {
            ...
            Action::UndoCommit => {
                try_or_popup!(self, "undo commit failed:", undo_last_commit(&self.repo.borrow()));
            }
        };
    ...
    fn delete_tag(&mut self, tag_name: String) -> Result<()> {
        if let Err(error) = sync::delete_tag(&self.repo.borrow(), &tag_name) {
            ...
            self.queue
                .push(InternalEvent::ConfirmAction(Action::DeleteRemoteTag(tag_name, remote)));
    ...
    fn delete_remote_branch(&mut self, branch_ref: &str) -> Result<()> {
        self.queue.push(
            ...
            branch_ref.rsplit('/').next().map_or_else(
                || InternalEvent::ShowErrorMsg(format!("Failed to find the branch name in {branch_ref}")),
