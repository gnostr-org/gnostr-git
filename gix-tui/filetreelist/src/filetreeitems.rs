...
impl FileTreeItems {
    ...
    fn show_element_upward(&mut self, index: usize) -> Option<usize> {
        ...
        let mut current_folder: (PathBuf, u8) = (item.info().full_path().parent()?.to_path_buf(), item.info().indent());

        let item_count = self.tree_items.len();
        for item in self.tree_items.iter_mut().rev().skip(item_count - index - 1) {
            if item.info().indent() == current_folder.1 {
                ...
                current_folder = (item.info().full_path().parent()?.to_path_buf(), item.info().indent());
            }
        }
    ...
    fn update_visibility(&mut self, prefix: &Option<PathBuf>, start_idx: usize, set_defaults: bool) {
        ...
        for i in start_idx..self.tree_items.len() {
            ...
            if prefix.as_ref().map_or(true, |prefix| item_path.starts_with(prefix)) {
            ...
            if item.kind().is_path() {
                ...
                if let Some(children) = children {
                    if *children == 1 {
                        ...
                        if items.get(i + 1).map(|item| item.kind().is_path()).unwrap_or_default() {
...
mod tests {
    ...
    fn test_push_path() {
        ...
        FileTreeItems::push_dirs(Path::new("a/b/c"), &mut items, &mut paths, &BTreeSet::new()).unwrap();

        assert_eq!(*paths.get(&Path::new("a")).unwrap(), 1);

        FileTreeItems::push_dirs(Path::new("a/b2/c"), &mut items, &mut paths, &BTreeSet::new()).unwrap();
    ...
    fn test_push_path2() {
        ...
        FileTreeItems::push_dirs(Path::new("a/b/d"), &mut items, &mut paths, &BTreeSet::new()).unwrap();
    ...
    fn test_indent() {
        ...
        let mut res = list.tree_items.iter().map(|i| (i.info().indent(), i.info().path()));
    ...
    fn test_indent_folder_file_name() {
        ...
        let list = FileTreeItems::new(&items, &BTreeSet::new()).unwrap();
        let mut res = list.tree_items.iter().map(|i| (i.info().indent(), i.info().path_str()));
...
mod test_merging {
    ...
    fn test_merge_single_paths() {
        ...
        let mut it = tree.iterate(0, 10).map(|(_, item)| item.info().full_path_str());
